...
async def create_warehouse_with_floors(w: WarehouseCreate):
    ...
    try:
        ...
        cur.execute(
            "INSERT INTO warehouses (name, location, width, height, depth, role, code, status, created_by) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING id",
            (w.name, w.location, w.width, w.height, w.depth, w.role, w.code, w.status, w.created_by)
        )
        ...
        for f in w.floors:
            ...
            if f.zones:
                for z in f.zones:
                    cur.execute(
                        "INSERT INTO zones (floor_id, zone_name, zone_type, width, depth, location_x, location_y, product_category) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id",
                        (floor_id, z.zone_name, z.zone_type, z.width, z.depth, z.location_x, z.location_y, z.product_category)
                    )
...
async def update_warehouse(warehouse_id: int, w: WarehouseUpdate, current_user_id: int):
    ...
    try:
        ...
        if w.role is not None:
            ...
            update_values.append(w.role)
        if w.code is not None:
            update_fields.append("code = %s")
            update_values.append(w.code)
        if w.status is not None:
            update_fields.append("status = %s")
            update_values.append(w.status)
...
async def delete_warehouse(warehouse_id: int, current_user_id: int):
    conn = get_conn()
    cur = conn.cursor()
    try:
        ...
        if not row:
            raise HTTPException(status_code=404, detail="Warehouse not found")
        if row['created_by'] != current_user_id:
            raise HTTPException(status_code=403, detail="Not authorized to delete this warehouse")

        # 1. Get all floors
        cur.execute("SELECT id FROM floors WHERE warehouse_id = %s", (warehouse_id,))
        floor_ids = [r['id'] for r in cur.fetchall()]

        if floor_ids:
            # 2. Get all zones on these floors
            cur.execute(f"SELECT id FROM zones WHERE floor_id IN ({','.join(['%s'] * len(floor_ids))})", floor_ids)
            zone_ids = [r['id'] for r in cur.fetchall()]

            if zone_ids:
                # 3. Delete shelves
                cur.execute(f"DELETE FROM shelves WHERE zone_id IN ({','.join(['%s'] * len(zone_ids))})", zone_ids)
                # 4. Delete zones
                cur.execute(f"DELETE FROM zones WHERE floor_id IN ({','.join(['%s'] * len(floor_ids))})", floor_ids)
            
            # 5. Delete areas
            cur.execute(f"DELETE FROM areas WHERE floor_id IN ({','.join(['%s'] * len(floor_ids))})", floor_ids)
            # 6. Delete floors
            cur.execute("DELETE FROM floors WHERE warehouse_id = %s", (warehouse_id,))

        # 7. Reset user's warehouse association
        cur.execute("UPDATE users SET warehouse_id = NULL WHERE warehouse_id = %s", (warehouse_id,))

        # 8. Delete warehouse
        cur.execute("DELETE FROM warehouses WHERE id = %s", (warehouse_id,))
        
        conn.commit()
        return {"status": "success", "message": "Warehouse and all associated data deleted"}
    except Exception as e:
        conn.rollback()
        raise HTTPException(status_code=500, detail=str(e))
    finally:
