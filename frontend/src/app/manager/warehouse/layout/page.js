...
import { Plus, Save, Trash2, ChevronRight, ChevronDown, Warehouse, Map, Box, ArrowLeft, Shield, Star, ToggleLeft, ToggleRight } from 'lucide-react';
...
export default function LayoutEntryPage() {
  const { state, fetchWarehouses, saveBulkShelves, deleteShelf, findNextAvailablePosition, toggleShelfStatus } = useWms();
  const { user } = useAuth();
  ...
  // Calculate the next available Aisle number based on current shelves
  const nextAisleNumber = useMemo(() => {
    if (!state.shelves || state.shelves.length === 0) return 1;

    const aisleNumbers = state.shelves
      .map(shelf => shelf.shelf_code?.split('-')[0]) // Get "A1", "A2"
      .filter(code => code?.startsWith('A'))         // Filter for Aisles
      .map(code => parseInt(code.replace('A', ''), 10)) // Convert to number
      .filter(num => !isNaN(num));                  // Remove fails

    const max = aisleNumbers.length > 0 ? Math.max(...aisleNumbers) : 0;
    return max + 1;
  }, [state.shelves]);
  ...
  const dynamicGapDefaults = useMemo(() => {
    return (state.shelfTypes || []).reduce((acc, type) => {
      acc[type.type_code] = type.default_gap;
      return acc;
    }, {});
  }, [state.shelfTypes]);

  const zoneObj = useMemo(() => {
    return state.zones?.find(z => z.id === selectedZone);
  ...
  useEffect(() => {
    ...
    if (zoneShelves.length > 0) {
        ...
        setBulkData({
            ...
            product_category: zoneObj?.product_category || ''
        });
        setGapEdited(false);
    } else {
        // No shelves yet in this zone: still respect existing zone.product_category
        setBulkData({
          ...initialBulkState,
          product_category: zoneObj?.product_category || ''
        });
  ...
  }, [selectedZone, state.shelves, initialBulkState, userEdited, dynamicGapDefaults, zoneObj]);
  ...
  const filteredShelves = useMemo(() => {
    return state.shelves
      .filter(s => s.zone_id == selectedZone)
      .sort((a, b) => {
        if (a.aisle_num !== b.aisle_num) {
          return a.aisle_num - b.aisle_num;
        }
        return a.bay_num - b.bay_num;
      });
  }, [state.shelves, selectedZone]);

  // Apply search by shelf code for table display only
  const shelvesMatchingQuery = useMemo(() => {
    const q = shelfQuery.trim().toLowerCase();
    if (!q) return filteredShelves;
    return filteredShelves.filter(s =>
      (s.shelf_code || "").toLowerCase().includes(q)
    );
  }, [filteredShelves, shelfQuery]);

  const groupedShelves = useMemo(() => {
    return shelvesMatchingQuery.reduce((acc, shelf) => {
      const aisle = shelf.aisle_num;
      if (!acc[aisle]) {
        acc[aisle] = [];
      }
      acc[aisle].push(shelf);
      return acc;
    }, {});
  }, [shelvesMatchingQuery]);
  ...
  const handleBulkChange = (e) => {
    ...
    if (name === 'aisle_gap') {
        ...
        setBulkData(prev => {
            ...
            if (!gapEdited) {
               newData.aisle_gap = dynamicGapDefaults[newType] || 3.0;
            }
            return newData;
        });
    } else {
        setBulkData(prev => ({
          ...prev,
          [name]: (name === 'shelf_type' || name === 'product_category') ? value : (parseFloat(value) || 0)
        }));
    }
  };
  ...
  const handleBulkSubmit = async (e) => {
    ...
    // SMART AISLE AUTO-INCREMENT (Non-Contiguous)
    // Check if the requested aisles are already in use in the database (state.shelves)
    // We want to find `num_aisles` free slots starting from `start_aisle`.
    
    let targetAisles = [];
    if (state.shelves && state.shelves.length > 0) {
        // Gather all used Aisle Numbers
        // EXCEPTION: Ignore shelves in the CURRENT ZONE (selectedZone) because we are about to overwrite them.
        // If user is editing Zone A, and Zone A has Aisle 1, we shouldn't treat Aisle 1 as "taken" by someone else.
        // We only care about collisions with OTHER zones.
        
        const otherShelves = state.shelves.filter(s => s.zone_id !== selectedZone);
        ...
        // Also check by Shelf Code parsing for robustness (e.g. A2-...)
        otherShelves.forEach(s => {
             if (s.shelf_code && s.shelf_code.startsWith('A')) {
                 const part = s.shelf_code.split('-')[0].substring(1); // "1" from "A1"
                 const num = parseInt(part, 10);
                 if (!isNaN(num)) usedAisles.add(num);
             }
        });

        // Loop to find N free aisle numbers
        let candidate = start_aisle;
        let found = 0;
        let attempts = 0;
        const maxAttempts = 2000;

        while (found < num_aisles && attempts < maxAttempts) {
            if (!usedAisles.has(candidate)) {
                targetAisles.push(candidate);
                found++;
            }
            candidate++;
            attempts++;
        ...
        // Notify user if the result isn't a simple contiguous block starting at requested
        // e.g. requested 1, 2. Found 1, 3.
        const expectedEnd = start_aisle + num_aisles - 1;
        const specificMsg = targetAisles.length > 0 ? targetAisles.join(', ') : 'None';
        
        // Simple check: if last element is different from expected end, or first from start
        // Actually simplest is: if we skipped any.
        const perfectSequence = Array.from({length: num_aisles}, (_, i) => start_aisle + i);
        const isPerfect = perfectSequence.every((val, i) => val === targetAisles[i]);

        if (!isPerfect) {
             console.log(`Auto-adjusted Aisles to avoid conflicts. targetAisles: ${specificMsg}`);
             // alert(`Notice: Some requested aisles were occupied. Will generate Aisles: ${specificMsg}`);
        }
    } else {
        // If no shelves exist, just standard sequence
        targetAisles = Array.from({length: num_aisles}, (_, i) => start_aisle + i);
    }
    ...
    const BACK_GAP = 0.2;
    const aislePairDepth = (2 * bay_depth) + aisle_gap;
    const totalLayoutDepth = (num_aisles * aislePairDepth) + (Math.max(0, num_aisles - 1) * BACK_GAP);

    // Position aisles from BOTTOM to TOP
    // Lowest aisle number (targetAisles[0]) starts at the bottom of the allocated space
    let yCursor = start_y + totalLayoutDepth - aislePairDepth;

    for (let a = 0; a < num_aisles; a++) {
      const aisleNum = targetAisles[a];
      ...
      for (let b = 0; b < oddCount; b++) {
        ...
        for (let l = 0; l < levels_per_bay; l++) {
          ...
          const isVariableHeight = ['SELECTIVE_PALLET', 'DRIVE_IN', 'CANTILEVER'].includes(shelf_type);
          let currentLevelHeight = level_height;
          let currentLevelZ = l * level_height;

          if (isVariableHeight) {
             const bottomExtra = 0.2;
             if (l === 0) {
                 currentLevelHeight = level_height + bottomExtra;
                 currentLevelZ = 0;
             } else {
                 currentLevelHeight = level_height;
                 currentLevelZ = (level_height + bottomExtra) + ((l - 1) * level_height);
             }
          }
          ...
          for (let bin = 1; bin <= bins_per_level; bin++) {
            ...
            if ((binX + binWidth <= zoneRight) && (topRowY + bay_depth <= zoneBottom)) {
              if (!overlapsAny(binX, topRowY, currentLevelZ, binWidth, bay_depth, currentLevelHeight, obstacles) && !overlapsAny(binX, topRowY, currentLevelZ, binWidth, bay_depth, currentLevelHeight, newShelves)) {
                newShelves.push({ 
        ...
        for (let l = 0; l < levels_per_bay; l++) {
          ...
          for (let bin = 1; bin <= bins_per_level; bin++) {
            ...
            if ((binX + binWidth <= zoneRight) && (bottomRowY + bay_depth <= zoneBottom)) {
              if (!overlapsAny(binX, bottomRowY, currentLevelZ, binWidth, bay_depth, currentLevelHeight, obstacles) && !overlapsAny(binX, bottomRowY, currentLevelZ, binWidth, bay_depth, currentLevelHeight, newShelves)) {
                newShelves.push({ 
                  zone_id: selectedZone, 
                  shelf_code: shelfCode,
                  shelf_type, 
      ...
      // Move yCursor UP for the next aisle (closer to start_y)
      yCursor = yCursor - aislePairDepth - BACK_GAP;
    }

    // Update Zone Category if changed
    if (product_category !== (zoneObj?.product_category || '')) {
      try {
        await fetch(`${API_BASE}/zones/${selectedZone}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ product_category })
        });
      } catch (err) {
        console.error("Failed to update zone category:", err);
      }
    }
  ...
  const toggleAisleExpansion = (aisleNum) => {
    setExpandedAisles(prev => ({
      ...prev,
      [aisleNum]: !prev[aisleNum]
    }));
  };
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="flex justify-between items-center mb-6">
        ...
        <Link href="/manager/warehouse" className="text-indigo-600 hover:text-indigo-800 flex items-center gap-2">
      ...
      <div className="flex-1 flex gap-6 overflow-hidden">
        ...
        <div className="flex-1 bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden">
          <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50 gap-3">
            <h2 className="font-semibold text-gray-700 flex items-center gap-2">
              <Box size={18} />
              <span>
                Shelves in {zoneObj?.zone_name || 'Selected Zone'}
                {zoneObj?.product_category && (
                  <span className="text-gray-500 font-normal ml-2 text-sm">(Category: {zoneObj.product_category})</span>
                )}
              </span>
            </h2>
            <div className="flex items-center gap-2">
              <div className="relative hidden sm:block">
                <input
                  ...
                  className="w-48 rounded-lg border border-gray-300 bg-white px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300"
                />
              </div>
              <button 
          ...
          <div className="flex-1 overflow-y-auto p-">
            {showForm ? (
              <div className="bg-gray-50 p-6 rounded-xl border border-gray-200 animate-in fade-in slide-in-from-bottom-4">
                ...
                <form onSubmit={handleBulkSubmit} className="grid grid-cols-12 gap-2">
                  ...
                  <div className="col-span-6 md:col-span-3">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Total Aisles</label>
                  ...
                  <div className="col-span-6 md:col-span-3">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Bays per Aisle</label>
                  ...
                  <div className="col-span-6 md:col-span-3">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Levels per Bay</label>
                  ...
                  <div className="col-span-6 md:col-span-3">
                    ...
                    <input type="text" name="product_category" value={bulkData.product_category} onChange={handleBulkChange} disabled={readonly} className="w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border disabled:bg-gray-100" placeholder="Optional" />
                  </div>
                  ...
                  <div className="col-span-6 md:col-span-3">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Bay Width</label>
                  ...
                  <div className="col-span-6 md:col-span-3">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Bay Depth</label>
                  ...
                  <div className="col-span-6 md:col-span-3">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Level Height</label>
                  ...
                  <div className="col-span-6 md:col-span-3">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Aisle Gap</label>
                  ...
                  {(() => {
                    ...
                    if (zone) {
                        const { num_aisles, bays_per_aisle, levels_per_bay, bay_width, bay_depth, level_height, aisle_gap, shelf_type } = bulkData;
                        ...
                        const isVariableHeight = ['SELECTIVE_PALLET', 'DRIVE_IN', 'CANTILEVER'].includes(shelf_type);
                        const bottomExtra = isVariableHeight ? 0.2 : 0;
                        const totalH = (levels_per_bay * level_height) + bottomExtra;
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  {/* <thead className="bg-gray-50">
                    ...
                    {Object.keys(groupedShelves).length > 0 ? (
                      Object.entries(groupedShelves).map(([aisleNum, shelvesInAisle]) => (
                        <tbody key={aisleNum} className="bg-white divide-y divide-gray-200">
                          <tr 
                            key={`aisle-summary-${aisleNum}`} 
                            onClick={() => toggleAisleExpansion(aisleNum)}
                            className="bg-gray-50 hover:bg-gray-100 cursor-pointer"
                          >
                            <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                              <div className="flex items-center gap-2">
                                {expandedAisles[aisleNum] ? <ChevronDown size={16} /> : <ChevronRight size={16} />}
                                Aisle {aisleNum} ({shelvesInAisle.length} shelves)
                              </div>
                            </td>
                            <td colSpan="4" className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                              {`Bays: ${Math.min(...shelvesInAisle.map(s => s.bay_num))} - ${Math.max(...shelvesInAisle.map(s => s.bay_num))}, Levels: ${Math.min(...shelvesInAisle.map(s => s.level_num))} - ${Math.max(...shelvesInAisle.map(s => s.level_num))}`}
                            </td>
                          </tr>
                          {expandedAisles[aisleNum] && (
                            <>
                              <tr className="bg-gray-100">
                                <th className="px-6 py-2 text-left text-xs font-medium text-gray-600 uppercase tracking-wider pl-12">Code</th>
                                <th className="px-6 py-2 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Position</th>
                                <th className="px-6 py-2 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Dimensions (WxDxH)</th>
                                <th className="px-6 py-2 text-left text-xs font-medium text-gray-600 uppercase tracking-wider">Max Weight</th>
                                <th className="px-6 py-2 text-center text-xs font-medium text-gray-600 uppercase tracking-wider">Status</th>
                              </tr>
                              {shelvesInAisle.map((shelf, idx) => (
                                <tr key={shelf.shelf_code || idx} className="hover:bg-gray-50">
                                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 pl-12">{shelf.shelf_code}</td>
                                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">Aisle {shelf.aisle_num}, Bay {shelf.bay_num}, Level {shelf.level_num}</td>
                                  ...
                                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                    <div className="flex items-center justify-center gap-2">
                                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${shelf.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                        {shelf.status}
                                      </span>
                                      {!readonly && (
                                        <button 
                                          onClick={(e) => { 
                                            e.stopPropagation(); // Prevent row expansion/collapse
                                            toggleShelfStatus(shelf.id, shelf.status); 
                                          }}
                                          className="text-indigo-600 hover:text-indigo-900"
                                        >
                                          {shelf.status === 'active' ? <ToggleRight size={20} /> : <ToggleLeft size={20} />}
                                        </button>
                                      )}
                                    </div>
                                  </td>
