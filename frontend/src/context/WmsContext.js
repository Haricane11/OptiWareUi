...
useEffect(() => {
  ...
  const updateZoneDimensions = async (zoneId, width, depth, x, y) => {
    setState(prev => ({
      ...prev,
      zones: prev.zones.map(z => z.id === zoneId ? { 
        ...
        width, 
        depth,
        location_x: x !== undefined ? x : z.location_x,
        location_y: y !== undefined ? y : z.location_y
      } : z)
    }));
    
    const updateData = { width, depth };
    if (x !== undefined) updateData.location_x = x;
    if (y !== undefined) updateData.location_y = y;
    
    await updateZone(zoneId, updateData);
  };

  const updateAreaDimensions = async (floorId, areaIndex, width, depth, height, x, y) => {
     const floor = state.floors.find(f => f.id === floorId);
     const area = floor?.areas?.[areaIndex];
     if (!area || !area.id) return;
      ...
      floors: prev.floors.map(f => {
        ...
        if (areas[areaIndex]) {
          areas[areaIndex] = { 
            ...
            depth, 
            height,
            location_x: x !== undefined ? x : areas[areaIndex].location_x,
            location_y: y !== undefined ? y : areas[areaIndex].location_y
          };
        }
        return { ...f, areas };
      })
    ...
    const updateData = { width, depth, height };
    if (x !== undefined) updateData.location_x = x;
    if (y !== undefined) updateData.location_y = y;
  ...
  const toggleShelfStatus = async (shelfId, currentStatus) => {
    const next = (currentStatus || '').toLowerCase() === 'active' ? 'inactive' : 'active';
    // Optimistic update
    setState(prev => ({
      ...prev,
      shelves: prev.shelves.map(s => s.id === shelfId ? { ...s, status: next } : s)
    }));
    const ok = await updateShelf(shelfId, { status: next });
    if (!ok) {
      // Revert if failed
      setState(prev => ({
        ...prev,
        shelves: prev.shelves.map(s => s.id === shelfId ? { ...s, status: currentStatus } : s)
      }));
    }
    return ok;
  };
  ...
  return (
    <WmsContext.Provider value={{ 
        ...
        addZone, saveBulkShelves, updateShelf, deleteShelf, updateZone, updateArea, toggleShelfStatus,
