...
export default function ShelfFactory({ type, width, height, depth, highlight, isBottom, label }) {
  ...
  switch (type) {
        ...
        <group>
          ...
          <group rotation={[tilt, 0, 0]}>
            ...
            <Box args={[width - 0.1, 0.1, 0.02]} position={[0, 0.05, depth / 2]}>
              <meshStandardMaterial color="#721010ff" />
            </Box>
          </group>
        </group>
      );
    ...
    // Add 'label' to your props
    case "BIN_SHELVING": {
      const bThick = 0.02;
      const fontSize = 0.05; // Adjust based on your scene scale
      ...
      return (
        <group>
          ...
          {/* 2. The Shelf Code (The Label) */}
          <Text
            position={[0, -height / 2 + bThick + 0.01, depth / 2 + 0.001]} // Positioned on the front edge
            fontSize={fontSize}
            color="black"
            anchorX="center"
            anchorY="bottom"
          >
            {label} {/* e.g., "A2-B1-L5-01" */}
          </Text>
        </group>
      );
    }
